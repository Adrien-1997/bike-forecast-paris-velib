...
# - Appelle la fonction d'entraînement avec adaptation des kwargs
...
def main() -> int:
    ...
    # 3) Déterminer le module d’entraînement
    module_name = os.getenv("TRAIN_MODULE")
    if not module_name:
        # ordre: core → tools → train → flat (compat)
        try:
            import service.core.forecast  # nouveau chemin recommandé
            module_name = "service.core.forecast"
        except ModuleNotFoundError:
            try:
                import service.tools.forecast  # ancien essai présent dans ta version
                module_name = "service.tools.forecast"
            except ModuleNotFoundError:
                try:
                    import service.core.forecast
                    module_name = "service.train.forecast"
                except ModuleNotFoundError:
                    import service.core.forecast  # layout aplati
