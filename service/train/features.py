...
from .cal_features import add_calendar_features, feature_cols as calfeat_cols

def _load_base_5min() -> pd.DataFrame:
    """
    Load the 5-min aggregated parquet (exports/velib.parquet) and normalize timestamps (UTC naive).
    Expected columns at least:
      - tbin_utc, stationcode, nb_velos_bin, capacity_bin, occ_ratio_bin
      - optional weather: temp_C, precip_mm, wind_mps
    """
    path = Path("exports/velib.parquet")
    if not path.exists():
        raise FileNotFoundError(
            "[features] Dataset not found: exports/velib.parquet. "
            "Run tools.export_training_base first."
        )
    df = pd.read_parquet(path)
    if df.empty:
        raise ValueError("[features] Empty parquet exports/velib.parquet.")
...
def build_training_frame(horizon_minutes: int = 15, lookback_days: int = 30):
    """
    Build the training frame from 5-min base.
    Target: y_nb = nb_velos_bin at +horizon_minutes (e.g., +15 → +3 bins).
    """
