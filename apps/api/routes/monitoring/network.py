...
router = APIRouter(prefix="/monitoring/network", tags=["monitoring:network"])
...
@router.get("/dynamics")
def network_dynamics(request: Request):
    ...
    uri = f"{mon.rstrip('/')}/network/dynamics.json"
    return _proxy_json(uri, request, ttl=60)

@router.get("/stations")
def network_stations(request: Request):
    mon = getattr(settings, "GCS_MONITORING_PREFIX", None)
    if not mon or not mon.startswith("gs://"):
        raise HTTPException(status_code=500, detail="Monitoring prefix not configured")
    uri = f"{mon.rstrip('/')}/network/stations.json"
