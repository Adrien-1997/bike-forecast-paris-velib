...
def occupancy_5min(snapshot_df: pd.DataFrame, with_weather: bool = True) -> pd.DataFrame:
    ...
    if with_weather:
        ...
        if w is not None and not w.empty:
            ...
            agg = agg.merge(w[["hour_utc","temp_C","precip_mm","wind_mps"]],
                            on="hour_utc", how="left")

        if agg[["temp_C","precip_mm","wind_mps"]].isna().any(axis=1).any():
            try:
                wf = fetch_forecast(pd.to_datetime(agg["hour_utc"].max()), 24)
                if wf is not None and not wf.empty:
                    wf["hour_utc"] = _to_utc_naive_floor_hour(wf["hour_utc"])
                    agg = agg.merge(wf[["hour_utc","temp_C","precip_mm","wind_mps"]],
                                    on="hour_utc", how="left", suffixes=("", "_fx"))
                    for c in ["temp_C","precip_mm","wind_mps"]:
                        if f"{c}_fx" in agg:
                            agg[c] = agg[c].fillna(agg[f"{c}_fx"])
                    agg.drop(columns=[c for c in ["temp_C_fx","precip_mm_fx","wind_mps_fx"] if c in agg],
