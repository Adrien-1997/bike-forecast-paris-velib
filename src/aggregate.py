...
def main(input_path: Path, output_path: Path):
    ...
    # 3) Merge météo (concat sécurisée)
    parts = []
    ...
    if parts:
        ...
        # Typage/normalisation des heures
        s = pd.to_datetime(weather["hour_utc"], utc=True, errors="coerce")
        weather["hour_utc"] = s.dt.floor("h").dt.tz_localize(None)
        ...
        weather = pd.DataFrame(columns=["hour_utc","temp_C","precip_mm","wind_mps"])
