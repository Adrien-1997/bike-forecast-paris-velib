...
// On attend soit un tableau de blocs, soit un objet { blocks: [...] }
type DictionaryPayload = { blocks?: DocBlock[] } | DocBlock[] | null;

export const getStaticProps: GetStaticProps<Props> = async () => {
  const payload = (await getDoc<DictionaryPayload>("dictionary")) ?? null;

  const blocks: DocBlock[] = Array.isArray(payload)
    ? payload
...
export default function DictionaryPage({ blocks, generatedAt }: Props) {
  return (
    <>
      <Head>
        <title>Docs — Data Dictionary</title>
        <meta
          name="description"
          content="Dictionnaire des données (champs, schémas, unités)."
        />
      </Head>
      ...
      <main style={{ padding: "2rem", maxWidth: 1000, margin: "0 auto" }}>
        ...
        <section style={{ marginTop: 20 }}>
          {(!blocks || !blocks.length) && (
            <div className="small" style={{ opacity: 0.7 }}>
              Aucun contenu n’a été renvoyé par <code>/monitoring/docs/dictionary</code>.
            </div>
          )}
...
function Block({ block }: { block: any }) {
  ...
  switch (block.type) {
    ...
    default: {
      // Fallback — affiche la structure JSON brute
      return (
        <pre
          style={{
            background: "#0b1220",
