...
export const getStaticProps: GetStaticProps<Props> = async () => {
  const payload = (await getDoc("methodology")) ?? {};
...
export default function MethodologyPage({ blocks, generatedAt }: Props) {
  return (
    <>
      <Head>
        <title>Docs — Methodology</title>
        <meta
          name="description"
          content="Méthodologie : sources, transformations, protocoles d’évaluation et de monitoring."
        />
      </Head>
      ...
      <main style={{ padding: "2rem", maxWidth: 1000, margin: "0 auto" }}>
        ...
        <section style={{ marginTop: 20 }}>
          {(!blocks || !blocks.length) && (
            <div className="small" style={{ opacity: 0.7 }}>
              Aucun contenu n’a été renvoyé par <code>/monitoring/docs/methodology</code>.
            </div>
          )}
...
/* ─────────────── Renderer identique à /docs/dictionary ─────────────── */
