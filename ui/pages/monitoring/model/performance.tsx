    ...
    <>
      ...
      <main style={{ padding: "2rem", maxWidth: 1280, margin: "0 auto" }}>
        ...
        <section style={{ marginTop: 16, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
          <Card inline>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ fontSize: 12, color: "#9CA3AF" }}>Horizon</div>
              <HorizonSelect
                value={h}
                options={manifest?.horizons ?? [15]}
                onChange={(v) => setH(v)}
              />
        ...
        <section style={{ marginTop: 24 }}>
          <h2 style={{ margin: "12px 0" }}>Lift quotidien</h2>
          {liftCurve ? (
            <Plot
            ...
            <Empty>Pas de courbe de lift.</Empty>
          )}
        </section>

        {/* Daily bars MAE (baseline vs modèle) */}
        <section style={{ marginTop: 24 }}>
          <h2 style={{ margin: "12px 0" }}>MAE par jour (Modèle vs Baseline)</h2>
          {dailyBars.length ? (
            <Plot
            ...
            <Empty>Pas de métriques quotidiennes.</Empty>
          )}
        </section>

        {/* By hour / By dow */}
        <section style={{ marginTop: 24 }}>
          <h2 style={{ margin: "12px 0" }}>Découpes — heure & jour de semaine</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 16 }}>
            <Card>
              <h3 style={{ margin: "6px 0 10px 0", fontSize: 16 }}>Par heure (locale)</h3>
              {byHourBars.length ? (
                <Plot
            ...
            <Card>
              <h3 style={{ margin: "6px 0 10px 0", fontSize: 16 }}>Par jour de semaine</h3>
              {byDowBars.length ? (
                <Plot
        ...
        <section style={{ marginTop: 24 }}>
          <h2 style={{ margin: "12px 0" }}>Histogramme des résidus (y_true - y_pred)</h2>
          {histData.length ? (
            <Plot
            ...
            <Empty>Pas d’histogramme disponible.</Empty>
          )}
          {hist?.n != null && (
            <div style={{ fontSize: 12, opacity: 0.7, marginTop: 6 }}>
              n = {fmtInt(hist.n)} points
            </div>
          )}
        </section>
        ...
        <section style={{ marginTop: 24, marginBottom: 40 }}>
          <h2 style={{ margin: "12px 0" }}>Stations — meilleurs / moins bons lifts</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 16 }}>
            <Card>
              <h3 style={{ margin: "6px 0 10px 0", fontSize: 16 }}>Top 20 (lift)</h3>
              {stationsTopBottom.tops.length ? (
                <StationList rows={stationsTopBottom.tops} kind="top" />
              ) : (
                <Empty>Pas de stations (échantillon trop faible).</Empty>
              )}
            </Card>
            <Card>
              <h3 style={{ margin: "6px 0 10px 0", fontSize: 16 }}>Bottom 20 (lift)</h3>
...
function Card({ children, inline = false }: { children: React.ReactNode; inline?: boolean }) {
  return (
    <div
      style={{
        border: "1px solid #374151",
  ...
  return (
    <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "grid", gap: 8 }}>
      {rows.map((r) => {
        ...
        return (
          <li key={r.station_id}>
            <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 110px 90px 70px", alignItems: "center", gap: 10 }}>
              <div style={{ overflow: "hidden" }}>
                <div style={{ whiteSpace: "nowrap", textOverflow: "ellipsis" }}>
                  <b>{r.station_id}</b>
                  <span style={{ opacity: 0.6 }}> · n={fmtInt(r.n)}</span>
                </div>
                <div style={{ height: 6, borderRadius: 999, background: "#111827", marginTop: 6, position: "relative", overflow: "hidden" }}>
                  <div
                    style={{
                      ...
                      height: "100%",
                      background: better ? "#10b981" : "#ef4444",
                    }}
                  />
                </div>
              </div>
              <div style={{ textAlign: "right" }}>
                <small>MAE base</small>
                <div style={{ fontWeight: 700 }}>{fmtNum(maeB, 2)}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <small>MAE modèle</small>
                <div style={{ fontWeight: 700 }}>{fmtNum(maeM, 2)}</div>
              </div>
              <div style={{ textAlign: "right", fontWeight: 700 }}>{fmtPct(liftPct, 1)}</div>
            </div>
          </li>
        );
      })}
