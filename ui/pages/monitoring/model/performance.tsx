...
function fmtPct(x?: number | null, digits = 1) { const v = Number(x); return Number.isFinite(v) ? `${v.toFixed(digits)}%` : "—"; }
function fmtNum(x?: number | null, digits = 2) { const v = Number(x); return Number.isFinite(v) ? v.toFixed(digits) : "—"; }
function fmtInt(x?: number | null) { const v = Number(x); return Number.isFinite(v) ? v.toLocaleString("fr-FR") : "—"; }
...
export default function ModelPerformancePage() {
  ...
  /* ───────── KPI BAR (remplace l’ancienne grille, tout le reste inchangé) ───────── */
  ...
  // ── Stations Top/Bottom lift (inchangé) ─────────────────────────
        ...
        <section className="mt-6">
          ...
          {liftCurve ? (
            <Plot
              ...
              layout={{
                autosize: true, height: 320, margin: { l: 54, r: 10, t: 10, b: 40 },
        ...
        <section className="mt-6">
          ...
          {dailyBars.length ? (
            <Plot
              ...
              layout={{
                barmode: "group",
                autosize: true, height: 360, margin: { l: 54, r: 10, t: 10, b: 60 },
            ...
            <div className="card">
              ...
              {byHourBars.length ? (
                <Plot
                  ...
                  layout={{
                    barmode: "group",
                    autosize: true, height: 320, margin: { l: 54, r: 10, t: 10, b: 36 },
                    yaxis: { title: { text: "MAE" } },
...
/* ───────────────────────── UI widgets ───────────────────────── */
function HorizonSelect({ value, options, onChange }: { value: number; options: number[]; onChange: (v: number) => void; }) {
  const opts = Array.isArray(options) && options.length ? options : [15];
  return (
    <div className="row" style={{ gap: 8 }}>
      <select value={value} onChange={(e) => onChange(Number(e.target.value))} className="select" style={{ background: "#0b1220" }}>
        {opts.map((o) => (<option key={o} value={o}>{o} min</option>))}
      </select>
            ...
            <div className="row" style={{ gap: 10, alignItems: "center" }}>
              <div style={{ flex: 1, overflow: "hidden" }}>
                ...
                <div className="bar mt-2">
                  <div className={`bar__fill ${better ? "" : "bar__fill--danger"}`} style={{ width: `${Math.max(0, Math.min(100, liftPct ?? 0))}%` }} aria-hidden />
                </div>
              </div>
