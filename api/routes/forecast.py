...
generated by the backend jobs and stored in GCS under:

    SERVING_FORECAST_PREFIX/h{H}/latest.json

Key ideas:
- Horizons are configured via the `FORECAST_SUPPORTED` env (e.g. "15,60").
- The API does **not** recompute forecasts; it only reads JSON from GCS.
...
try:
    ...
    # Côté API, l'accès GCS est obligatoire : on lève une erreur explicite
    # si le client n'est pas disponible au démarrage.
...
def _proxy_json(gs_uri: str, request: Request, ttl: int = 60) -> JSONResponse:
    """Generic "GCS JSON → HTTP JSON" proxy with basic caching headers.
    ...
    ------
    HTTPException
        404 if the document is missing; 502 on GCS / JSON errors.
    """
    try:
...
def forecast_available():
    """Return metadata about available forecast horizons and GCS prefix.
...
def forecast_latest(
    ...
    """
    Proxy the latest forecast JSON for a given horizon from GCS.
    ...
    -------
    fastapi.responses.JSONResponse
        Sanitized JSON payload proxied from GCS, with `Cache-Control` set.
    """
    _validate_h(h)
